...
function createlist(material, region, speed, height, length){
    ...
    for(var i=0; i<coasterName.length; i++){
        if(material==constructionMaterial[i]&&region==parkRegion[i]&&Math.abs(rollerSpeed[i] - speed) < 30&&Math.abs(rollerHeight[i] - height) < 20&&Math.abs(rollerLength[i] - length) < 500){
